              ...
              <section className="settings-view" aria-labelledby="settings-title">
                ...
                <div className="settings-grid">
                  ...
                  <section className="settings-panel settings-permissions"><div className="settings-panel-heading"><div><h2>{locale === 'vi' ? 'Quyền dữ liệu' : 'Data permissions'}</h2><p>{locale === 'vi' ? 'Mục đích, thời hạn lưu và mọi thay đổi đồng ý đều được trình bày riêng.' : 'Purpose, retention, and every consent change are shown separately.'}</p></div>{settingsBusy && <LoaderCircle className="inline-spinner" size={18} />}</div><div className="permission-list">
                    <div className="permission-row permission-static"><span><strong>{locale === 'vi' ? 'Dữ liệu học tập cốt lõi' : 'Core study data'}</strong><small>{locale === 'vi' ? 'Tệp gốc tự xóa sau 30 ngày; dữ liệu đã xác nhận và hoạt động cá nhân được giữ tới khi bạn xóa tài khoản.' : 'Raw files expire after 30 days; confirmed records and personal activity remain until account deletion.'}</small></span><span className="permission-state active">{locale === 'vi' ? 'Đang dùng' : 'In use'}</span></div>
                    ...
                    <label className="permission-row"><span><strong>{locale === 'vi' ? 'Nghiên cứu tổng hợp' : 'Aggregate research'}</strong><small>{locale === 'vi' ? 'Chỉ hoạt động phát sinh sau khi bật mới đủ điều kiện cho số liệu tổng hợp, không định danh. Rút lại sẽ loại toàn bộ hoạt động của bạn khỏi tập nghiên cứu.' : 'Only activity created after opt-in is eligible for anonymized aggregates. Revoking removes all of your activity from the research set.'}</small></span><input type="checkbox" checked={consentByPurpose.research_metrics?.granted ?? false} disabled={settingsBusy} onChange={(event) => void updateConsent('research_metrics', event.target.checked)} /></label>
                  ...
                  <section className="settings-panel"><div className="settings-panel-heading"><div><h2>{locale === 'vi' ? 'Xuất dữ liệu' : 'Export your data'}</h2><p>{locale === 'vi' ? 'Tải một tệp JSON gồm dữ liệu có cấu trúc, kế hoạch, hoạt động cá nhân và lịch sử đồng ý. Tệp gốc không được kèm theo.' : 'Download a JSON file with your structured data, plans, personal activity, and consent history. Original files are not included.'}</p></div></div><button className="secondary-button" type="button" disabled={exportBusy} aria-busy={exportBusy} onClick={() => void downloadExport()}><Download size={17} /> {locale === 'vi' ? 'Tải bản xuất dữ liệu' : 'Download export'}</button></section>
